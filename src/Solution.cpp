...
namespace pacs {
     ...
     * @param exact Exact solution.
     */
    Solution::Solution(const Mesh &mesh, const Vector<Real> &numerical, const TriFunctor &exact, const Real &t):
    x{mesh.entries}, y{mesh.entries}, numerical{mesh.entries}, exact{mesh.entries} {
        ...
        for(std::size_t j = 0; j < mesh.elements.size(); ++j) {
            ...
            for(std::size_t k = 0; k < triangles.size(); ++k) {
                ...
                // Exact solution.
                Vector<Real> local_exact = exact(physical_x, physical_y, t);

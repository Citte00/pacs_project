...
namespace pacs {
    ...
    HeatEstimator::HeatEstimator(const Mesh &mesh,
        ...
        // Coefficients.
        Vector<Real> f_modals = evaluateSourceHeat(mesh, source, t, D);
        Vector<Real> g_modals = evaluateCoeff(mesh, dirichlet, t);

        Vector<Real> f_coeff = (norm(f_modals) > TOLERANCE) ? solve(mass, f_modals, blocks, DB) : Vector<Real>{mesh.dofs()};
        Vector<Real> g_coeff = (norm(g_modals) > TOLERANCE) ? solve(mass, g_modals, blocks, DB) : Vector<Real>{mesh.dofs()};
        ...
        for(std::size_t j = 0; j < mesh.elements.size(); ++j) {
            ...
            for(std::size_t k = 0; k < triangles.size(); ++k) {
                ...
                Vector<Real> f = source(physical_x, physical_y, t, D_ext);

                // Local source approximation.
                Vector<Real> f_bar = phi * f_coeff(indices);

                // Local estimator, R_{K, E}^2.
                this->estimates[j] += sizes[j] * sizes[j] * dot(scaled, (f_bar + lap_uh - partial_uh) * (f_bar + lap_uh - partial_uh));
            ...
            for(std::size_t k = 0; k < element_neighbours.size(); ++k) {
                ...
                if(neighbour == -1) { // Boundary edge.
                    ...
                    Vector<Real> g = dirichlet(physical_x, physical_y, t);
                    auto [grad_g_x, grad_g_y] = dirichlet_gradient(physical_x, physical_y, t);

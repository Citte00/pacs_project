 ...
 */
std::array<Matrix<Real>, 3> basis_2d(const Mesh &mesh, const std::size_t &index,
                                     const std::array<Vector<Real>, 2> &nodes) {
  ...
  for (std::size_t j = 0; j < x_degree.size(); ++j) {
    ...
    Real coefficient =
        std::sqrt((2.0 * x_degree[j] + 1.0) * (2.0 * y_degree[j] + 1.0)) / 2.0;
  ...
  // Scaling.
  for (std::size_t j = 0; j < gradx_phi_evaluations.m_rows; ++j) {
    for (std::size_t k = 0; k < gradx_phi_evaluations.m_columns; ++k) {
      Vector<Real> node{2};
 ...
 * @return Matrix<Real>
 */
Matrix<Real> lap_basis_2d(const Mesh &mesh, const std::size_t &index,
                          const std::array<Vector<Real>, 2> &nodes) {
  ...
  // Scaling.
  for (std::size_t j = 0; j < lap_phi_x_evaluations.m_rows; ++j) {
    for (std::size_t k = 0; k < lap_phi_x_evaluations.m_columns; ++k) {
      Vector<Real> node{2};

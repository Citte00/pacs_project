...
FKPPEstimator::FKPPEstimator(const DataFKPP &data, const Mesh &mesh,
  ...
  for (std::size_t j = 0; j < mesh.elements.size(); ++j) {
    ...
    for (std::size_t k = 0; k < triangles.size(); ++k) {
      ...
      Vector<Real> f = data.source_f(physical_x, physical_y, t, D_ext, alpha);

      // Local source approximation.
      Vector<Real> f_bar = phi * f_coeff(indices);

      // Local non-linear term approx.
      Vector<Real> non_lin_uh = alpha * local_uh * (1 - local_uh);
  ...
  this->estimate = std::sqrt(this->estimate);
     ...
     * @param ost 
     */
    void FKPPEstimator::print(std::ostream &ost) const {

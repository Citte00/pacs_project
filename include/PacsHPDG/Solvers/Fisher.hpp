...
#pragma omp parallel for schedule(dynamic)
    for (std::size_t j = 0; j < num_elem; ++j) {
      ...
      for (const auto &triangle : triangles) {
        ...
        Vector<Real> Dext = data_.D_ext(physical_x, physical_y, 0.0);
        Vector<Real> alpha = data_.alpha(physical_x, physical_y, 0.0);
...
#pragma omp parallel for schedule(dynamic)
    for (std::size_t j = 0; j < num_elem; ++j) {
      ...
      for (const auto &triangle : triangles) {
        ...
        // Param initialization.
        Vector<Real> alpha = data_.alpha(physical_x, physical_y, 0.0);
        Vector<Real> c_star = ch_(indices);
...
#pragma omp parallel for schedule(dynamic)
    for (std::size_t j = 0; j < num_elem; ++j) {
      ...
      for (const auto &triangle : triangles) {
        ...
        Vector<Real> Dext = data_.D_ext(physical_x, physical_y, this->m_t);
        Vector<Real> alpha = data_.alpha(physical_x, physical_y, this->m_t);
      ...
      for (std::size_t k = 0; k < element_neighbours.size(); ++k) {
        ...
        // Boundary conditions.
        Vector<Real> boundary = data_.DirBC(physical_x, physical_y, this->m_t);
...
#pragma omp critical
  ...
  Vector<T> solver(const DataFKPP &data_, const Mesh &mesh_,
    ...
    Sparse<Real> LHS = this->m_mass + data_.dt * data_.theta * this->m_stiff;
    Sparse<Real> RHS =
    ...
    if (data_.theta == 0) {
      ...
      Sparse<Real> MN = assembly_nl(data_, mesh_, this->m_ch_old);
      ...
      Vector<Real> c_star = 1.5 * this->m_ch_old - 0.5 * ch_oold_;
      Sparse<Real> MN = assembly_nl(data_, mesh_, c_star);
...
#pragma omp parallel for schedule(dynamic)
    for (std::size_t j = 0; j < num_elem; ++j) {
      ...
      for (const auto &triangle : triangles) {
        ...
        // function solution.
        Vector<Real> D_ext = data_.D_ext(physical_x, physical_y, this->m_t);
        Vector<Real> alpha = data_.alpha(physical_x, physical_y, this->m_t);

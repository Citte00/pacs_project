...
# Ticks.
degree_ticks = [degrees[0], degrees[-1]]
...
# Labels.
degree_labels = [f"{tick:.3f}" for tick in degree_ticks]
...
if energy_errors:
    fig.suptitle("L2, DG and Energy errors vs. degree")
else:
    fig.suptitle("L2 and DG errors vs. degree")

# L2.
axes[0].plot(degrees, l2_errors, color=black, marker="*", linewidth=1, label="$L^2$ error") # Error.

# DG.
axes[1].plot(degrees, dg_errors, color=black, marker="*", linewidth=1, label="$DG$ error") # Error.

# Energy error.
if energy_errors:
    axes[2].plot(degrees, energy_errors, color=black, marker="*", linewidth=1, label="Energy error")  # Error.
